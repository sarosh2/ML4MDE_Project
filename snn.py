...
class SNN:
    ...
    def train(self, X, Y, t):
        ...
        for node in self.input_nodes:
            self.vertices[node].forward_prop(cp.array(X))
        ...
        output = SNN.softmax(output)
        gradient = output - Y


        for i in range(len(self.output_nodes) - 1, -1, -1):
            #print(y_train[i, :].shape)
            #self.vertices[self.output_nodes[i]].output = output[i, :].reshape(1, output.shape[1])
        ...
        #calculate loss
        loss = SNN.categorical_crossentropy(output, Y)
        #print("Loss: ", cp.mean(loss))

        #calculate accuracy
        accuracy = SNN.caclulate_accuracy(output, Y)
    ...
    def execute(self, epochs, batch_size, x_train, y_train, x_val, y_val, x_test, y_test):
        ...
        for i in range(epochs):
            ...
            metrics = []
            for batch_num, (batch_X, batch_Y) in enumerate(SNN.get_batches(x_train, y_train, batch_size)):
            ...
            #validate output
            for node in self.input_nodes:
                self.vertices[node].forward_prop(cp.array(x_val))

            output = self.vertices[self.output_nodes[0]].output

            for j in range(1, 10):
                output = cp.vstack([output, self.vertices[self.output_nodes[j]].output])
            val_loss = cp.mean(SNN.categorical_crossentropy(output, y_val))
            val_accuracy = cp.mean(SNN.caclulate_accuracy(output, y_val))
        ...
        for node in self.input_nodes:
            self.vertices[node].forward_prop(cp.array(x_test))
        ...
        test_loss = cp.mean(SNN.categorical_crossentropy(output, y_test))
        test_accuracy = cp.mean(SNN.caclulate_accuracy(output, y_test))
